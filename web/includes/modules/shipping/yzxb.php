<?php
// 2. /includes/modules/shipping/usu.php
/**
 * ECSHOP 邮政小包快递插件
*/
$shipping_lang = ROOT_PATH.'languages/' .$GLOBALS['_CFG']['lang']. '/shipping/yzxb.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    $modules[$i]['author']  = 'gy';
    /* 插件作者的官方网站 */
    $modules[$i]['website'] = 'www.higoshop.com';
    /* 配送接口需要的参数 */
    $modules[$i]['configure'] = array(
                                    array('name' => 'item_fee',     'value'=>20),    /* 单件商品配送的价格 */
                                    array('name' => 'base_fee',    'value'=>10),    /* 500克以内的价格 */
                                    array('name' => 'step_fee',     'value'=>5),    /* 续重每500克增加的价格 */
                                );
    /* 模式编辑器 */
    $modules[$i]['print_model'] = 2;
    /* 打印单背景 */
    $modules[$i]['print_bg'] = '/images/receipt/dly_yzxb.jpg';
   /* 打印快递单标签位置信息 */
...
class yzxb
{
     ...
     * @return  decimal
     */
    function calculate($goods_weight, $goods_amount, $goods_number,$force=0)
    {
        //强制付邮费 多少都不免邮
        if ($this->configure['free_money'] > 0 && $goods_amount >= $this->configure['free_money'] && $force == 0)
        {
            return 0;
        }
        else
    ...
    function query($invoice_sn)
    {
        $str = '<form style="margin:0px" methods="post" '.
            'action="http://www.usu.cn/bill.asp" name="queryForm_' .$invoice_sn. '" target="_blank">'.
